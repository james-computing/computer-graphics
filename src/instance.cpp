...
void Instance::createUniformBuffers(Core & core) {
    size_t const MAX_FRAMES_IN_FLIGHT {core.getMaxFramesInFlight()};
...
void Instance::updateUniformBuffer(Core & core, uint32_t currentImage) {
...
void Instance::createDescriptorSets(Core & core) {
    ...
    std::vector<vk::DescriptorSetLayout> const descriptorSetLayouts {std::vector(core.getMaxFramesInFlight(), *core.getDescriptorSetLayout())};

    // Allocate descriptor sets
    vk::DescriptorSetAllocateInfo const descriptorSetAllocateInfo {
        .descriptorPool = descriptorPool,
        .descriptorSetCount = core.getMaxFramesInFlight(),
        .pSetLayouts = descriptorSetLayouts.data()
    };

    descriptorSets = core.getLogicalDevice().allocateDescriptorSets(descriptorSetAllocateInfo);
    ...
    for (size_t i {0}; i < MAX_FRAMES_IN_FLIGHT; ++i) {
        ...
        core.getLogicalDevice().updateDescriptorSets(writeDescriptorSets, {});
    }
}